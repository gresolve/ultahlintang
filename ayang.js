kecepatan = 27;kecepatan2=30;ftganti=0;fungsi=0;ftfungsi=0;fungsiAwal=0;fungsitimer=0;deffotostiker=fotostiker.src;Content.style = "opacity:1;margin-top:30vh;";wallpaper.style="opacity:1";
  ...
  async function mulainama() {
           ...
           setTimeout(bqmuncul,700);
  }
  
  function ftmuncul(){
    if(ftganti==0){fotostiker.src = deffotostiker;}
    if(ftganti==1){fotostiker.src = fotostiker2.src;}
    if(ftganti==2){fotostiker.src = fotostiker4.src;}
    if(ftganti==3){fotostiker.src = fotostiker5.src;}
    if(ftganti==4){fotostiker.src = fotostiker8.src;}
    if(ftganti==5){fotostiker.src = fotostiker2.src;}
  ...
  function ketAkhir(){
  new TypeIt("#kalimat", {
  strings: ["" + vakhiran], startDelay: 70, speed: 20, waitUntilVisible: true,
  afterComplete: function(){
  ...
  kalimat.innerHTML = "";
  new TypeIt("#kalimat", {
  strings: [vketik1], startDelay: 110, speed: kecepatan, waitUntilVisible: true,
  afterComplete: function(){
  ...
  if (opsLcheck !== jumlahP) {
    ...
    ftganti += 1;fthilang();setTimeout(ftmuncul, 500);
    opsLcheck+=1;//wpcheck();
...
document.addEventListener("DOMContentLoaded", function () {
    // Fungsi efek ketik
    function ketikElemen(idElemen, speed = 50, startDelay = 0) {
      let el = document.getElementById(idElemen);
      if (!el) return;
  
      let teksAsli = el.innerHTML.trim(); // Simpan teks asli
      el.innerHTML = ""; // Kosongkan isi awal
    ...
    function mulaiAnimasi() {
      document.getElementById("kalimat").classList.remove("sembunyi");
  
      setTimeout(() => {
        document.getElementById("kalimat1").classList.remove("sembunyi");
      }, 2000);
  
      setTimeout(() => {
        document.getElementById("kalimat4").classList.remove("sembunyi");
      }, 4000);
  
      setTimeout(() => {
        document.getElementById("kalimat5").classList.remove("sembunyi");
      }, 15000);
  
      // Tampilkan dan ketik kalimat6ngetik setelah 15 detik
      setTimeout(() => {
        let el = document.getElementById("kalimat6ngetik");
        el.classList.remove("sembunyi");
        ketikElemen("kalimat6ngetik", 50, 0);
      }, 15000);
    }
  
    // Klik awal dari user
    let tombolAwal = document.querySelector("#loveIn label");
    if (tombolAwal) {
      tombolAwal.addEventListener("click", function () {
        document.getElementById("loveIn").classList.add("sembunyi");
        mulaiAnimasi();
      });
    }
  });
